...
async function handleGenerateTitleCommand(host: SlashCommandHost): Promise<void> {
  ...
  try {
    ...
    if (title === undefined) {
      host.showError(
        'Session title generation unavailable: make sure the conversation has prompts to generate from, and set [session_title].model to a resolvable model when a specific one is wanted.',
      );
      return;
    }
