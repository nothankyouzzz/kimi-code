...
AI session title generation. When enabled, clients request a short title for the session once the first turn completes, and the rename field or `/title generate` can force a regeneration at any time. Without it, sessions keep the local first-prompt excerpt as the title.

| Field | Type | Default | Description |
| --- | --- | --- | --- |
| `enabled` | `boolean` | `false` | Enable AI session title generation. Can also be enabled via `KIMI_CODE_EXPERIMENTAL_AUTO_SESSION_TITLE=1` (or the master `KIMI_CODE_EXPERIMENTAL_FLAG=1`) |
